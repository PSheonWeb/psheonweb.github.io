...
collapsibleOpen(element)});if(options.accordion||collapsible_type==="accordion"||collapsible_type===undefined){collapsibleOpen($panel_headers.filter('.active').first(),!0)}else{$panel_headers.filter('.active').each(function(){collapsibleOpen($(this),!0)})}})};$(document).ready(function(){$('.collapsible').collapsible()})})(jQuery);(function($){$.fn.scrollTo=function(elem){$(this).scrollTop($(this).scrollTop()-$(this).offset().top+$(elem).offset().top);return this};$.fn.dropdown=function(options){var defaults={inDuration:300,outDuration:225,constrainWidth:!0,hover:!1,gutter:0,belowOrigin:!1,alignment:'left',stopPropagation:!1};if(options==="open"){this.each(function(){$(this).trigger('open')});return!1}
...
$this.children("img").one("load",function(){updateParallax(!0)}).each(function(){if(this.complete)$(this).trigger("load")});$(window).scroll(function(){window_width=$(window).width();updateParallax(!1)});$(window).resize(function(){window_width=$(window).width();updateParallax(!1)})})}})(jQuery);(function($){var methods={init:function(options){var defaults={onShow:null,swipeable:!1,responsiveThreshold:Infinity};options=$.extend(defaults,options);var namespace=Materialize.objectSelectorString($(this));return this.each(function(i){var uniqueNamespace=namespace+i;var $this=$(this),window_width=$(window).width();var $active,$content,$links=$this.find('li.tab a'),$tabs_width=$this.width(),$tabs_content=$(),$tabs_wrapper,$tab_width=Math.max($tabs_width,$this[0].scrollWidth)/$links.length,$indicator,index=0,prev_index=0,clicked=!1,clickedTimeout,transition=300;var calcRightPos=function(el){return Math.ceil($tabs_width-el.position().left-el[0].getBoundingClientRect().width-$this.scrollLeft())};var calcLeftPos=function(el){return Math.floor(el.position().left+$this.scrollLeft())};var animateIndicator=function(prev_index){if(index-prev_index>=0){$indicator.velocity({"right":calcRightPos($active)},{duration:transition,queue:!1,easing:'easeOutQuad'});$indicator.velocity({"left":calcLeftPos($active)},{duration:transition,queue:!1,easing:'easeOutQuad',delay:90})}else{$indicator.velocity({"left":calcLeftPos($active)},{duration:transition,queue:!1,easing:'easeOutQuad'});$indicator.velocity({"right":calcRightPos($active)},{duration:transition,queue:!1,easing:'easeOutQuad',delay:90})}};if(options.swipeable){if(window_width>options.responsiveThreshold){options.swipeable=!1}}
...
if(!$this.find('.indicator').length){$this.append('<li class="indicator"></li>')}
...
$(window).off('resize.tabs-'+uniqueNamespace).on('resize.tabs-'+uniqueNamespace,function(){$tabs_width=$this.width();$tab_width=Math.max($tabs_width,$this[0].scrollWidth)/$links.length;if(index<0){index=0}
if($tab_width!==0&&$tabs_width!==0){$indicator.css({"right":calcRightPos($active)});$indicator.css({"left":calcLeftPos($active)})}});if(options.swipeable){$links.each(function(){var $curr_content=$(Materialize.escapeHash(this.hash));$curr_content.addClass('carousel-item');$tabs_content=$tabs_content.add($curr_content)});$tabs_wrapper=$tabs_content.wrapAll('<div class="tabs-content carousel"></div>');$tabs_content.css('display','');$('.tabs-content.carousel').carousel({fullWidth:!0,noWrap:!0,onCycleTo:function(item){if(!clicked){var prev_index=index;index=$tabs_wrapper.index(item);$active.removeClass('active');$active=$links.eq(index);$active.addClass('active');animateIndicator(prev_index);if(typeof options.onShow==="function"){options.onShow.call($this[0],$content)}}}})}else{$links.not($active).each(function(){$(Materialize.escapeHash(this.hash)).hide()})}
$this.off('click.tabs').on('click.tabs','a',function(e){if($(this).parent().hasClass('disabled')){e.preventDefault();return}
if(!!$(this).attr("target")){return}
clicked=!0;$tabs_width=$this.width();$tab_width=Math.max($tabs_width,$this[0].scrollWidth)/$links.length;$active.removeClass('active');var $oldContent=$content;$active=$(this);$content=$(Materialize.escapeHash(this.hash));$links=$this.find('li.tab a');var activeRect=$active.position();$active.addClass('active');prev_index=index;index=$links.index($(this));if(index<0){index=0}
if(options.swipeable){if($tabs_content.length){$tabs_content.carousel('set',index,function(){if(typeof options.onShow==="function"){options.onShow.call($this[0],$content)}})}}else{if($content!==undefined){$content.show();$content.addClass('active');if(typeof options.onShow==="function"){options.onShow.call(this,$content)}}
if($oldContent!==undefined&&!$oldContent.is($content)){$oldContent.hide();$oldContent.removeClass('active')}}
clickedTimeout=setTimeout(function(){clicked=!1},transition);animateIndicator(prev_index);e.preventDefault()})})},select_tab:function(id){this.find('a[href="#'+id+'"]').trigger('click')}};$.fn.tabs=function(methodOrOptions){if(methods[methodOrOptions]){return methods[methodOrOptions].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof methodOrOptions==='object'||!methodOrOptions){return methods.init.apply(this,arguments)}else{$.error('Method '+methodOrOptions+' does not exist on jQuery.tabs')}};$(document).ready(function(){$('ul.tabs').tabs()})})(jQuery);(function($){$.fn.tooltip=function(options){var timeout=null,margin=5;var defaults={delay:350,tooltip:'',position:'bottom',html:!1};if(options==="remove"){this.each(function(){$('#'+$(this).attr('data-tooltip-id')).remove();$(this).removeAttr('data-tooltip-id');$(this).off('mouseenter.tooltip mouseleave.tooltip')});return!1}
...
tooltipEl.css({top:newCoordinates.y,left:newCoordinates.x});scaleXFactor=Math.SQRT2*tooltipWidth/parseInt(backdropOffsetWidth);scaleYFactor=Math.SQRT2*tooltipHeight/parseInt(backdropOffsetHeight);scaleFactor=Math.max(scaleXFactor,scaleYFactor);tooltipEl.velocity({translateY:tooltipVerticalMovement,translateX:tooltipHorizontalMovement},{duration:350,queue:!1}).velocity({opacity:1},{duration:300,delay:50,queue:!1});backdrop.css({visibility:'visible'}).velocity({opacity:1},{duration:55,delay:0,queue:!1}).velocity({scaleX:scaleFactor,scaleY:scaleFactor},{duration:300,delay:0,queue:!1,easing:'easeInOutQuad'})};timeoutRef=setTimeout(showTooltip,tooltipDelay)},'mouseleave.tooltip':function(){started=!1;clearTimeout(timeoutRef);setTimeout(function(){if(started!==!0){tooltipEl.velocity({opacity:0,translateY:0,translateX:0},{duration:225,queue:!1});backdrop.velocity({opacity:0,scaleX:1,scaleY:1},{duration:225,queue:!1,complete:function(){backdrop.css({visibility:'hidden'});tooltipEl.css({visibility:'hidden'});started=!1}})}},225)}})})};var repositionWithinScreen=function(x,y,width,height){var newX=x;var newY=y;if(newX<0){newX=4}else if(newX+width>window.innerWidth){newX-=newX+width-window.innerWidth}
if(newY<0){newY=4}else if(newY+height>window.innerHeight+$(window).scrollTop){newY-=newY+height-window.innerHeight}
return{x:newX,y:newY}};$(document).ready(function(){$('.tooltipped').tooltip()})})(jQuery);(function(window){'use strict';var Waves=Waves||{};var $$=document.querySelectorAll.bind(document);function isWindow(obj){return obj!==null&&obj===obj.window}
...
element.addEventListener('mousedown',showEffect,!1)};window.Waves=Waves;document.addEventListener('DOMContentLoaded',function(){Waves.displayEffect()},!1)})(window);(function($,Vel){'use strict';var _defaults={displayLength:Infinity,inDuration:300,outDuration:375,className:undefined,completeCallback:undefined,activationPercent:0.8};var Toast=function(){function Toast(message,displayLength,className,completeCallback){_classCallCheck(this,Toast);if(!message){return}
this.options={displayLength:displayLength,className:className,completeCallback:completeCallback};this.options=$.extend({},Toast.defaults,this.options);this.message=message;this.panning=!1;this.timeRemaining=this.options.displayLength;if(Toast._toasts.length===0){Toast._createContainer()}
Toast._toasts.push(this);var toastElement=this.createToast();toastElement.M_Toast=this;this.el=toastElement;this._animateIn();this.setTimer()}
_createClass(Toast,[{key:'createToast',value:function createToast(){var toast=document.createElement('div');toast.classList.add('toast');if(this.options.className){var classes=this.options.className.split(' ');var i=void 0,count=void 0;for(i=0,count=classes.length;i<count;i++){toast.classList.add(classes[i])}}
...
if(options.edge=='left'){menu.css('transform','translateX(-100%)');$dragTarget.css({'left':0})}else{menu.addClass('right-aligned').css('transform','translateX(100%)');$dragTarget.css({'right':0})}
if(menu.hasClass('fixed')){if(window.innerWidth>992){menu.css('transform','translateX(0)')}}
if(menu.hasClass('fixed')){$(window).resize(function(){if(window.innerWidth>992){if($('#sidenav-overlay').length!==0&&menuOut){removeMenu(!0)}else{menu.css('transform','translateX(0%)')}}else if(menuOut===!1){if(options.edge==='left'){menu.css('transform','translateX(-100%)')}else{menu.css('transform','translateX(100%)')}}})}
if(options.closeOnClick===!0){menu.on("click.itemclick","a:not(.collapsible-header)",function(){if(!(window.innerWidth>992&&menu.hasClass('fixed'))){removeMenu()}})}
var removeMenu=function(restoreNav){panning=!1;menuOut=!1;$('body').css({overflow:'',width:''});$('#sidenav-overlay').velocity({opacity:0},{duration:200,queue:!1,easing:'easeOutQuad',complete:function(){$(this).remove()}});if(options.edge==='left'){$dragTarget.css({width:'',right:'',left:'0'});menu.velocity({'translateX':'-100%'},{duration:200,queue:!1,easing:'easeOutCubic',complete:function(){if(restoreNav===!0){menu.removeAttr('style');menu.css('width',options.menuWidth)}}})}else{$dragTarget.css({width:'',right:'0',left:''});menu.velocity({'translateX':'100%'},{duration:200,queue:!1,easing:'easeOutCubic',complete:function(){if(restoreNav===!0){menu.removeAttr('style');menu.css('width',options.menuWidth)}}})}
if(typeof options.onClose==='function'){options.onClose.call(this,menu)}};var panning=!1;var menuOut=!1;if(options.draggable){$dragTarget.on('click',function(){if(menuOut){removeMenu()}});$dragTarget.hammer({prevent_default:!1}).on('pan',function(e){if(e.gesture.pointerType=="touch"){var direction=e.gesture.direction;var x=e.gesture.center.x;var y=e.gesture.center.y;var velocityX=e.gesture.velocityX;if(x===0&&y===0){return}
...
if(options.edge==='left'){if(x<options.menuWidth/2){menuOut=!1}
else if(x>=options.menuWidth/2){menuOut=!0}
menu.css('transform','translateX('+(x-options.menuWidth)+'px)')}else{if(x<window.innerWidth-options.menuWidth/2){menuOut=!0}
else if(x>=window.innerWidth-options.menuWidth/2){menuOut=!1}
var rightPos=x-options.menuWidth/2;if(rightPos<0){rightPos=0}
menu.css('transform','translateX('+rightPos+'px)')}
var overlayPerc;if(options.edge==='left'){overlayPerc=x/options.menuWidth;$overlay.velocity({opacity:overlayPerc},{duration:10,queue:!1,easing:'easeOutQuad'})}else{overlayPerc=Math.abs((x-window.innerWidth)/options.menuWidth);$overlay.velocity({opacity:overlayPerc},{duration:10,queue:!1,easing:'easeOutQuad'})}}}).on('panend',function(e){if(e.gesture.pointerType=="touch"){var $overlay=$('#sidenav-overlay');var velocityX=e.gesture.velocityX;var x=e.gesture.center.x;var leftPos=x-options.menuWidth;var rightPos=x-options.menuWidth/2;if(leftPos>0){leftPos=0}
if(rightPos<0){rightPos=0}
panning=!1;if(options.edge==='left'){if(menuOut&&velocityX<=0.3||velocityX<-0.5){if(leftPos!==0){menu.velocity({'translateX':[0,leftPos]},{duration:300,queue:!1,easing:'easeOutQuad'})}
$overlay.velocity({opacity:1},{duration:50,queue:!1,easing:'easeOutQuad'});$dragTarget.css({width:'50%',right:0,left:''});menuOut=!0}else if(!menuOut||velocityX>0.3){$('body').css({overflow:'',width:''});menu.velocity({'translateX':[-1*options.menuWidth-10,leftPos]},{duration:200,queue:!1,easing:'easeOutQuad'});$overlay.velocity({opacity:0},{duration:200,queue:!1,easing:'easeOutQuad',complete:function(){if(typeof options.onClose==='function'){options.onClose.call(this,menu)}
$(this).remove()}});$dragTarget.css({width:'10px',right:'',left:0})}}else{if(menuOut&&velocityX>=-0.3||velocityX>0.5){if(rightPos!==0){menu.velocity({'translateX':[0,rightPos]},{duration:300,queue:!1,easing:'easeOutQuad'})}
$overlay.velocity({opacity:1},{duration:50,queue:!1,easing:'easeOutQuad'});$dragTarget.css({width:'50%',right:'',left:0});menuOut=!0}else if(!menuOut||velocityX<-0.3){$('body').css({overflow:'',width:''});menu.velocity({'translateX':[options.menuWidth+10,rightPos]},{duration:200,queue:!1,easing:'easeOutQuad'});$overlay.velocity({opacity:0},{duration:200,queue:!1,easing:'easeOutQuad',complete:function(){if(typeof options.onClose==='function'){options.onClose.call(this,menu)}
$(this).remove()}});$dragTarget.css({width:'10px',right:0,left:''})}}}})}
$this.off('click.sidenav').on('click.sidenav',function(){if(menuOut===!0){menuOut=!1;panning=!1;removeMenu()}else{var $body=$('body');var $overlay=$('<div id="sidenav-overlay"></div>');var oldWidth=$body.innerWidth();$body.css('overflow','hidden');$body.width(oldWidth);$('body').append($dragTarget);if(options.edge==='left'){$dragTarget.css({width:'50%',right:0,left:''});menu.velocity({'translateX':[0,-1*options.menuWidth]},{duration:300,queue:!1,easing:'easeOutQuad'})}else{$dragTarget.css({width:'50%',right:'',left:0});menu.velocity({'translateX':[0,options.menuWidth]},{duration:300,queue:!1,easing:'easeOutQuad'})}
$overlay.css('opacity',0).click(function(){menuOut=!1;panning=!1;removeMenu();$overlay.velocity({opacity:0},{duration:300,queue:!1,easing:'easeOutQuad',complete:function(){$(this).remove()}})});$('body').append($overlay);$overlay.velocity({opacity:1},{duration:300,queue:!1,easing:'easeOutQuad',complete:function(){menuOut=!0;panning=!1}});if(typeof options.onOpen==='function'){options.onOpen.call(this,menu)}}
return!1})})},destroy:function(){var $overlay=$('#sidenav-overlay');var $dragTarget=$('.drag-target[data-sidenav="'+$(this).attr('data-activates')+'"]');$overlay.trigger('click');$dragTarget.remove();$(this).off('click');$overlay.remove()},show:function(){this.trigger('click')},hide:function(){$('#sidenav-overlay').trigger('click')}};$.fn.sideNav=function(methodOrOptions){if(methods[methodOrOptions]){return methods[methodOrOptions].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof methodOrOptions==='object'||!methodOrOptions){return methods.init.apply(this,arguments)}else{$.error('Method '+methodOrOptions+' does not exist on jQuery.sideNav')}}})(jQuery);(function($){var jWindow=$(window);var elements=[];var elementsInView=[];var isSpying=!1;var ticks=0;var unique_id=1;var offset={top:0,right:0,bottom:0,left:0};function findElements(top,right,bottom,left){var hits=$();$.each(elements,function(i,element){if(element.height()>0){var elTop=element.offset().top,elLeft=element.offset().left,elRight=elLeft+element.width(),elBottom=elTop+element.height();var isIntersect=!(elLeft>right||elRight<left||elTop>bottom||elBottom<top);if(isIntersect){hits.push(element)}}});return hits}
...
var multiple=$select.attr('multiple')?!0:!1,lastID=$select.attr('data-select-id');if(lastID){$select.parent().find('span.caret').remove();$select.parent().find('input').remove();$select.unwrap();$('ul#select-options-'+lastID).remove()}
...
$active.find('img').each(function(){$active.find('.caption').velocity({opacity:1,translateX:0,translateY:0},{duration:options.transition,queue:!1,easing:'easeOutQuad'})});$interval=setInterval(function(){$active_index=$slider.find('.active').index();moveToSlide($active_index+1)},options.transition+options.interval);var panning=!1;var swipeLeft=!1;var swipeRight=!1;$this.hammer({prevent_default:!1}).on('pan',function(e){if(e.gesture.pointerType==="touch"){clearInterval($interval);var direction=e.gesture.direction;var x=e.gesture.deltaX;var velocityX=e.gesture.velocityX;var velocityY=e.gesture.velocityY;$curr_slide=$slider.find('.active');if(Math.abs(velocityX)>Math.abs(velocityY)){$curr_slide.velocity({translateX:x},{duration:50,queue:!1,easing:'easeOutQuad'})}
if(direction===4&&(x>$this.innerWidth()/2||velocityX<-0.65)){swipeRight=!0}
...
if($(e.target).is($('.card-reveal .card-title'))||$(e.target).is($('.card-reveal .card-title i'))){$(this).find('.card-reveal').velocity({translateY:0},{duration:225,queue:!1,easing:'easeInOutQuad',complete:function(){$(this).css({display:'none'});$card.css('overflow',$card.data('initialOverflow'))}})}else if($(e.target).is($('.card .activator'))||$(e.target).is($('.card .activator i'))){$card.css('overflow','hidden');$(this).find('.card-reveal').css({display:'block'}).velocity("stop",!1).velocity({translateY:'-100%'},{duration:300,queue:!1,easing:'easeInOutQuad'})}}})})})(jQuery);(function($){var materialChipsDefaults={data:[],placeholder:'',secondaryPlaceholder:'',autocompleteOptions:{}};$(document).ready(function(){$(document).on('click','.chip .close',function(e){var $chips=$(this).closest('.chips');if($chips.attr('data-initialized')){return}
...
var curr_options=$.extend({},materialChipsDefaults,options);self.hasAutocomplete=!$.isEmptyObject(curr_options.autocompleteOptions.data);this.init=function(){var i=0;var chips;self.$el.each(function(){var $chips=$(this);var chipId=Materialize.guid();self.chipId=chipId;if(!curr_options.data||!(curr_options.data instanceof Array)){curr_options.data=[]}
$chips.data('chips',curr_options.data);$chips.attr('data-index',i);$chips.attr('data-initialized',!0);if(!$chips.hasClass(self.SELS.CHIPS)){$chips.addClass('chips')}
...
swipeLeft=!1;swipeRight=!1}})})})})(jQuery);(function($){var scrollFireEventsHandled=!1;Materialize.scrollFire=function(options){var onScroll=function(){var windowScroll=window.pageYOffset+window.innerHeight;for(var i=0;i<options.length;i++){var value=options[i];var selector=value.selector,offset=value.offset,callback=value.callback;var currentElement=document.querySelector(selector);if(currentElement!==null){var elementOffset=currentElement.getBoundingClientRect().top+window.pageYOffset;if(windowScroll>elementOffset+offset){if(value.done!==!0){if(typeof callback==='function'){callback.call(this,currentElement)}else if(typeof callback==='string'){var callbackFunc=new Function(callback);callbackFunc(currentElement)}
value.done=!0}}}}};var throttledScroll=Materialize.throttle(function(){onScroll()},options.throttle||100);if(!scrollFireEventsHandled){window.addEventListener("scroll",throttledScroll);window.addEventListener("resize",throttledScroll);scrollFireEventsHandled=!0}
setTimeout(throttledScroll,0)}})(jQuery);(function(factory){Materialize.Picker=factory(jQuery)})(function($){var $window=$(window);var $document=$(document);var $html=$(document.documentElement);function PickerConstructor(ELEMENT,NAME,COMPONENT,OPTIONS){if(!ELEMENT)return PickerConstructor;var IS_DEFAULT_THEME=!1,STATE={id:ELEMENT.id||'P'+Math.abs(~~(Math.random()*new Date()))},SETTINGS=COMPONENT?$.extend(!0,{},COMPONENT.defaults,OPTIONS):OPTIONS||{},CLASSES=$.extend({},PickerConstructor.klasses(),SETTINGS.klass),$ELEMENT=$(ELEMENT),PickerInstance=function(){return this.start()},P=PickerInstance.prototype={constructor:PickerInstance,$node:$ELEMENT,start:function(){if(STATE&&STATE.start)return P;STATE.methods={};STATE.start=!0;STATE.open=!1;STATE.type=ELEMENT.type;ELEMENT.autofocus=ELEMENT==getActiveElement();ELEMENT.readOnly=!SETTINGS.editable;ELEMENT.id=ELEMENT.id||STATE.id;if(ELEMENT.type!='text'){ELEMENT.type='text'}
...
keycode=event.keyCode,keycodeToMove=P.component.key[keycode],target=event.target;if(keycode==27){P.close(!0)}
...
else if(!P.$root.find('.'+CLASSES.highlighted).hasClass(CLASSES.disabled)){P.set('select',P.component.item.highlight);if(SETTINGS.closeOnSelect){P.close(!0)}}}
...
else if(!targetDisabled&&'pick' in targetData){P.set('select',targetData.pick);if(SETTINGS.closeOnSelect){P.close(!0)}}
else if(targetData.clear){P.clear();if(SETTINGS.closeOnSelect){P.close(!0)}}else if(targetData.close){P.close(!0)}});aria(P.$root[0],'hidden',!0)}
...
return!1};DatePicker.prototype.flipEnable=function(val){var itemObject=this.item;itemObject.enable=val||(itemObject.enable==-1?1:-1)};DatePicker.prototype.deactivate=function(type,datesToDisable){var calendar=this,disabledItems=calendar.item.disable.slice(0);if(datesToDisable=='flip'){calendar.flipEnable()}else if(datesToDisable===!1){calendar.flipEnable(1);disabledItems=[]}else if(datesToDisable===!0){calendar.flipEnable(-1);disabledItems=[]}
else{datesToDisable.map(function(unitToDisable){var matchFound;for(var index=0;index<disabledItems.length;index+=1){if(calendar.isDateExact(unitToDisable,disabledItems[index])){matchFound=!0;break}}
if(!matchFound){if(_.isInteger(unitToDisable)||_.isDate(unitToDisable)||$.isArray(unitToDisable)||$.isPlainObject(unitToDisable)&&unitToDisable.from&&unitToDisable.to){disabledItems.push(unitToDisable)}}})}
return disabledItems};DatePicker.prototype.activate=function(type,datesToEnable){var calendar=this,disabledItems=calendar.item.disable,disabledItemsCount=disabledItems.length;if(datesToEnable=='flip'){calendar.flipEnable()}else if(datesToEnable===!0){calendar.flipEnable(1);disabledItems=[]}else if(datesToEnable===!1){calendar.flipEnable(-1);disabledItems=[]}
else{datesToEnable.map(function(unitToEnable){var matchFound,disabledUnit,index,isExactRange;for(index=0;index<disabledItemsCount;index+=1){disabledUnit=disabledItems[index];if(calendar.isDateExact(disabledUnit,unitToEnable)){matchFound=disabledItems[index]=null;isExactRange=!0;break}
...
if(override=="short_months")if(selectedObject!=null)return monthsCollection[selectedObject.month];else return monthsCollection[viewsetObject.month];return _.node('div',monthsCollection[viewsetObject.month],settings.klass.month)},createYearLabel=function(override){var focusedYear=viewsetObject.year,numberYears=settings.selectYears===!0?5:~~(settings.selectYears/2);if(numberYears){var minYear=minLimitObject.year,maxYear=maxLimitObject.year,lowestYear=settings.selectYearSub?focusedYear-~~(settings.selectYearSub):focusedYear-numberYears;highestYear=settings.selectOnlyPrev?focusedYear+~~(settings.selectOnlyPrev):focusedYear+numberYears;if(minYear>lowestYear){highestYear+=minYear-lowestYear;lowestYear=minYear}
...
count=images.length;function setupEvents(){if(typeof window.ontouchstart!=='undefined'){view.on('touchstart.carousel',tap);view.on('touchmove.carousel',drag);view.on('touchend.carousel',release)}
view.on('mousedown.carousel',tap);view.on('mousemove.carousel',drag);view.on('mouseup.carousel',release);view.on('mouseleave.carousel',release);view.on('click.carousel',click)}
...
if(!noWrap||center>=0&&center<count){el=images[wrap(center)];if(!$(el).hasClass('active')){view.find('.carousel-item').removeClass('active');$(el).addClass('active')}
el.style[xform]=alignment+' translateX('+ -delta/2+'px)'+' translateX('+dir*options.shift*tween*i+'px)'+' translateZ('+options.dist*tween+'px)';el.style.zIndex=0;if(options.fullWidth){tweenedOpacity=1}else{tweenedOpacity=1-0.2*tween}
...
if(!noWrap||center+i<count){el=images[wrap(center+i)];el.style[xform]=alignment+' translateX('+(options.shift+(dim*i-delta)/2)+'px)'+' translateZ('+zTranslation+'px)';el.style.zIndex=-i;el.style.opacity=tweenedOpacity;el.style.display='block'}
...
if(!noWrap||center-i>=0){el=images[wrap(center-i)];el.style[xform]=alignment+' translateX('+(-options.shift+(-dim*i-delta)/2)+'px)'+' translateZ('+zTranslation+'px)';el.style.zIndex=-i;el.style.opacity=tweenedOpacity;el.style.display='block'}}
if(!noWrap||center>=0&&center<count){el=images[wrap(center)];el.style[xform]=alignment+' translateX('+ -delta/2+'px)'+' translateX('+dir*options.shift*tween+'px)'+' translateZ('+options.dist*tween+'px)';el.style.zIndex=0;if(options.fullWidth){tweenedOpacity=1}else{tweenedOpacity=1-0.2*tween}
el.style.opacity=tweenedOpacity;el.style.display='block'}
...
if(typeof oneTimeCallback==="function"){oneTimeCallback.call(this,$curr_item,dragged);oneTimeCallback=null}}
...
function click(e){if(dragged){e.preventDefault();e.stopPropagation();return!1}else if(!options.fullWidth){var clickedIndex=$(e.target).closest('.carousel-item').index();var diff=wrap(center)-clickedIndex;if(diff!==0){e.preventDefault();e.stopPropagation()}
cycleTo(clickedIndex)}}
function cycleTo(n){var diff=center%count-n;if(!noWrap){if(diff<0){if(Math.abs(diff+count)<Math.abs(diff)){diff+=count}}else if(diff>0){if(Math.abs(diff-count)<diff){diff-=count}}}
if(diff<0){view.trigger('carouselNext',[Math.abs(diff)])}else if(diff>0){view.trigger('carouselPrev',[diff])}}
function tap(e){if(e.type==='mousedown'&&$(e.target).is('img')){e.preventDefault()}
pressed=!0;dragged=!1;vertical_dragged=!1;reference=xpos(e);referenceY=ypos(e);velocity=amplitude=0;frame=offset;timestamp=Date.now();clearInterval(ticker);ticker=setInterval(track,100)}
...
function release(e){if(pressed){pressed=!1}else{return}
clearInterval(ticker);target=offset;if(velocity>10||velocity<-10){amplitude=0.9*velocity;target=offset+amplitude}
target=Math.round(target/dim)*dim;if(noWrap){if(target>=dim*(count-1)){target=dim*(count-1)}else if(target<0){target=0}}
...
return!0});var throttledResize=Materialize.throttle(function(){if(options.fullWidth){item_width=view.find('.carousel-item').first().innerWidth();var imageHeight=view.find('.carousel-item.active').height();dim=item_width*2+options.padding;offset=center*2*item_width;target=offset;setCarouselHeight(!0)}else{scroll()}},200);$(window).off('resize.carousel-'+uniqueNamespace).on('resize.carousel-'+uniqueNamespace,throttledResize);setupEvents();scroll(offset);$(this).on('carouselNext',function(e,n,callback){if(n===undefined){n=1}
if(typeof callback==="function"){oneTimeCallback=callback}
target=dim*Math.round(offset/dim)+dim*n;if(offset!==target){amplitude=target-offset;timestamp=Date.now();requestAnimationFrame(autoScroll)}});$(this).on('carouselPrev',function(e,n,callback){if(n===undefined){n=1}
if(typeof callback==="function"){oneTimeCallback=callback}
target=dim*Math.round(offset/dim)-dim*n;if(offset!==target){amplitude=target-offset;timestamp=Date.now();requestAnimationFrame(autoScroll)}});$(this).on('carouselSet',function(e,n,callback){if(n===undefined){n=0}
if(typeof callback==="function"){oneTimeCallback=callback}
